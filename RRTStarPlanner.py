...
class RRTStarPlanner(object):
    ...
    def plan(self):
        ...
        while not (self.tree.is_goal_exists(self.goal) and self.stop_on_goal) and time.time()-start < 12.5:
    ...
    def extend(self, x_near, x_rand):
        if self.ext_mode == "E1":
            ...
            if self.bb.compute_distance(x_near, self.goal) < self.max_step_size/2:
        ...
        if self.k is None:
            ...
            d = len(new_config)
            k = max((5*d*int((math.log(i)/i)**(1/d)), 1))
        else:
...
class RRTStarPlanner(object):
    ...
    def plan(self):
        ...
        start = time.time()
        #while not (self.tree.is_goal_exists(self.goal) and self.stop_on_goal) and itrs <self.max_itr:
        while not (self.tree.is_goal_exists(self.goal) and self.stop_on_goal) and time.time()-start < 60:
            rand_config = self.bb.sample_random_config(self.goal_prob, self.goal)
            # print(rand_config)
            self.extend(self.tree.get_nearest_config(rand_config)[1], rand_config)
            itrs+=1
